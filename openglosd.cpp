...
bool cOglFb::Init(void) {
    ...
    if(fbstatus != GL_FRAMEBUFFER_COMPLETE) {
        esyslog("[softhddev]ERROR: Framebuffer is not complete (0x%08x)!\n", fbstatus);
        return false;
    }
    return true;
}
...
bool cOglOutputFb::Init(void) {
    ...
    GL_CHECK(fbstatus = glCheckFramebufferStatus(GL_FRAMEBUFFER));
    if(fbstatus != GL_FRAMEBUFFER_COMPLETE) {
        esyslog("[softhddev]ERROR::cOglOutputFb: Framebuffer is not complete (0x%08x)!", fbstatus);
        return false;
    }
    return true;
}
...
void cOglThread::Action(void) {
    ...
    Debug(3, "[softhddev]OpenGL Context initialized");
    ...
    Debug(3, "[softhddev]Shaders initialized");
    ...
    Debug(3, "[softhddev]vdpau interop initialized");
    ...
    Debug(3, "[softhddev]Vertex buffers initialized");

    GL_CHECK(glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxTextureSize));
    Debug(3, "[softhddev]Maximum Pixmap size: %dx%dpx", maxTextureSize, maxTextureSize);
    ...
    Debug(3, "[softhddev]Cleaning up OpenGL stuff");
    Cleanup();
    Debug(3, "[softhddev]OpenGL Worker Thread Ended");
}
...
bool cOglThread::InitOpenGL(void) {
    ...
    Debug(3, "[softhddev]OpenGL using display %s", displayName);
...
cOglOsd::cOglOsd(int Left, int Top, uint Level, std::shared_ptr<cOglThread> oglThread) : cOsd(Left, Top, Level) {
    ...
    Debug(3, "[softhddev]cOglOsd osdLeft %d osdTop %d screenWidth %d screenHeight %d", Left, Top, osdWidth, osdHeight);
...
void cOglOsd::Flush(void) {
    ...
    //clear buffer
    //uint64_t start = cTimeMs::Now();
    //Debug(3, "[softhddev]Start Flush at %" PRIu64 "", cTimeMs::Now());
    ...
    //Debug(3, "[softhddev]End Flush at %" PRIu64 ", duration %d", cTimeMs::Now(), (int)(cTimeMs::Now()-start));
}
